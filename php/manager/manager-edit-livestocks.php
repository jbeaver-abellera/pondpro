...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare(
        "UPDATE livestocks SET breed = ?, crayfish_count = ?, age = ?, tank_or_pond_location = ?, harvest_date = ? WHERE product_id = ?"
    );
    $stmt->bind_param(
        "sisssi",
        $_POST['breed'],
    ...
    header("Location: manager-livestocks.php");
    exit();
}
?>
...
<body>
    <div class="form-container">
        <?php if (!empty($result)) : ?>
            ...
            <form action="manager-edit-livestocks.php" method="POST">
